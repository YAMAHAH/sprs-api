...
import com.spsrexpress.apiproxy.utils.DataMapUtil;
...
//        JSONObject jsonObject = xmlConvertUtil.xmlToJSONObject(res);
//        xmlConvertUtil.getValue(jsonObject,"root.Result.RC");
        return res;
    }

    @PostMapping(path="/waLogin",produces = "application/json")
    public Map<String,Object> waLogin() throws IOException{
        return xmlConvertUtil.xmlToMap(this.login());
    }

    private String logout(String sId) throws IOException{
    ...
    public String createInvoice(@ApiParam(name = "reqParam", value = "JSON或XML格式的参数数据", required = true)
        ...
        String res="{ \"result\": \"0\" }";
        if( contentType.equalsIgnoreCase("application/xml")){
            res = httpRequestUtil.postRequest(getRequestUrl(), getCreateInvoiceAction(secureId,contractNumber,reqParam));
        }else if(contentType.equalsIgnoreCase("application/json")){
            String reqXml = xmlConvertUtil.jsonToXML(reqParam);
            reqXml = getCreateInvoiceAction(secureId,contractNumber,reqXml);
            res = httpRequestUtil.postRequest(getRequestUrl(), reqXml);
        }
        return xmlConvertUtil.xmlToJson(res);
    }
